...
try:
    ...
    class SandboxBackendProtocol:  # type: ignore
        pass
...
async def simple_cli(
    ...
    console.print("[dim]Starting CLI loop...[/dim]")
...
async def _run_agent_session(
    ...
    console.print("[dim]Calculating baseline tokens...[/dim]")
    try:
        baseline_tokens = calculate_baseline_tokens(model, agent_dir, system_prompt, assistant_id)
    except Exception as e:
        console.print(f"[yellow]Warning: Baseline token calculation failed: {e}[/yellow]")
        baseline_tokens = 0
...
async def main(
    ...
    console.print(f"[dim]Model created: {settings.model_provider} / {settings.model_name}[/dim]")
