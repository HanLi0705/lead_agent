...
def calculate_baseline_tokens(model, agent_dir: Path, system_prompt: str, assistant_id: str) -> int:
    ...
    try:
        # For local models (like Qwen), get_num_tokens_from_messages might hang
        # because tiktoken doesn't recognize the model name and might try to 
        # download encodings or fail slowly.
