...
def create_model(model_name_override: str | None = None) -> BaseChatModel:
    ...
    if model_name_override:
        ...
        if provider == "openai" and not settings.has_openai:
            ...
            # Local Qwen models from superagent don't require external API keys
            pass
    ...
    elif os.environ.get("QWEN_MODEL") or os.environ.get("USE_LOCAL_QWEN") == "true":
        provider = "qwen"
        model_name = os.environ.get("QWEN_MODEL", "qwen3:235b")
    else:
    ...
    if provider == "qwen":
        try:
            # Add workspace root to sys.path to ensure superagent is importable
            import sys
            from pathlib import Path

            # __file__ is /.../han/deepagents-cli/deepagents_cli/config.py
            # parent.parent.parent is /.../han/
            root_path = Path(__file__).parent.parent.parent.resolve()
            if str(root_path) not in sys.path:
                sys.path.insert(0, str(root_path))

            from superagent.llm import build_qwen_llm, build_qwenvl_llm

            if "vl" in model_name.lower():
                return build_qwenvl_llm()
            return build_qwen_llm()
        except ImportError as e:
            console.print(f"[bold red]Error:[/bold red] Could not import superagent.llm: {e}")
            console.print(
                f"[dim]Attempted to add {root_path} to sys.path[/dim]"
            )
            sys.exit(1)
