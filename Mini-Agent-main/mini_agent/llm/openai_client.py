...
class OpenAIClient(LLMClientBase):
    ...
    def __init__(
        ...
        Args:
            ...
            ssl_ca_path: Path to CA bundle/PEM file
            retry_config: Optional retry configuration
        """
        super().__init__(api_key, api_base, model, verify, ssl_ca_path, retry_config)

        # Create custom httpx client for SSL verification settings
        # This is needed for local models with self-signed certs
        verify_setting: Any = verify
        if verify and ssl_ca_path:
            verify_setting = ssl_ca_path
    ...
    async def _make_api_request(
        ...
        # Enable reasoning_split ONLY for MiniMax API to separate thinking content
        # For other providers, this might cause errors
        if "minimax" in self.api_base.lower():
    ...
    def _parse_response(self, response: Any) -> LLMResponse:
        ...
        # Extract thinking content from reasoning_details (MiniMax) or reasoning_content (Standard)
        thinking_content = ""
        
        # 1. Check reasoning_content (Standard OpenAI-compatible thinking field)
        if hasattr(message, "reasoning_content") and message.reasoning_content:
            thinking_content = message.reasoning_content
            
        # 2. Check reasoning_details (MiniMax specific)
