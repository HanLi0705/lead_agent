...
{Colors.BOLD}{Colors.BRIGHT_YELLOW}Available Commands:{Colors.RESET}
  ...
  {Colors.BRIGHT_GREEN}/skill{Colors.RESET}     - List available Claude skills
...
async def run_agent(workspace_dir: Path):
    ...
    command_completer = WordCompleter(
        ["/help", "/clear", "/history", "/stats", "/skill", "/skills", "/log", "/exit", "/quit", "/q"],
        ignore_case=True,
        sentence=True,
    )
        ...
        try:
            ...
            if user_input.startswith("/"):
                ...
                if command in ["/exit", "/quit", "/q"]:
                    ...
                    if not skill_loader or not skill_loader.loaded_skills:
                        print(f"\n{Colors.YELLOW}⚠️  No skills loaded or skills are disabled.{Colors.RESET}\n")
                        continue

                    print(f"\n{Colors.BOLD}{Colors.BRIGHT_CYAN}Available Claude Skills:{Colors.RESET}")
                    print(f"{Colors.DIM}{'─' * 60}{Colors.RESET}")
                    for skill in skill_loader.loaded_skills.values():
                        print(f"  {Colors.BRIGHT_GREEN}• {skill.name}{Colors.RESET}")
                        print(f"    {Colors.DIM}{skill.description}{Colors.RESET}")
                    print(f"{Colors.DIM}{'─' * 60}{Colors.RESET}")
                    print(f"{Colors.DIM}Agent will load these skills on-demand to handle specific tasks.{Colors.RESET}\n")
                    continue
