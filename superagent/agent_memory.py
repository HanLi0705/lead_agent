...
This module implements a hierarchical memory system that loads agent memory from
persistent storage and injects it into the system prompt. The memory system supports
two layers: user memory (applies globally) and project memory (project-specific).
"""
...
class AgentMemoryState(AgentState):
    ...
    This state manages two types of memory:
    - user_memory: Personal preferences from ~/.superagent/{agent}/ (applies everywhere)
    - project_memory: Project-specific context (loaded from project root)
    """

    user_memory: NotRequired[str]
    """Personal preferences from ~/.superagent/{agent}/ (applies everywhere)."""
...
# This follows the hierarchical pattern from Claude Code:
...
- `[project-root]/.superagent/agent.md` (preferred)
...
- **At the start of ANY new session**: Check both user and project memories
  ...
  - Project: `ls {project_superagent_dir}` (if in a project)
...
1. User asks a question → Check project directory first: `ls {project_superagent_dir}`
2. If relevant files exist → Read them with `read_file '{project_superagent_dir}/[filename]'`
...
### Project Agent File: `{project_superagent_dir}/agent.md`
...
### Project Memory Files: `{project_superagent_dir}/*.md`
...
**Examples:**
- `{project_superagent_dir}/api-design.md` - REST API patterns used
- `{project_superagent_dir}/architecture.md` - System architecture overview
- `{project_superagent_dir}/deployment.md` - How to deploy this project
...
```
ls {project_superagent_dir}                          # List project memory files
read_file '{project_superagent_dir}/agent.md'        # Read project instructions
edit_file '{project_superagent_dir}/agent.md' ...    # Update project instructions
write_file '{project_superagent_dir}/agent.md' ...  # Create project memory file
```

**Important**:
- Project memory files are stored in `.superagent/` inside the project root
...
class AgentMemoryMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            project_root: Optional project root directory for project memory.
        ...
        # User paths
        self.agent_dir = self._get_agent_dir(assistant_id)
        # Store both display path (with ~) and absolute path for file operations
        self.agent_dir_display = f"~/.superagent/{assistant_id}"
        self.agent_dir_absolute = str(self.agent_dir)

        self.system_prompt_template = system_prompt_template or DEFAULT_MEMORY_SNIPPET

    def _get_agent_dir(self, assistant_id: str) -> Path:
    ...
    def _get_user_agent_md_path(self) -> Path:
        ...
        Returns:
            Path to the user agent.md file.
        """
        return self.agent_dir / "agent.md"

    def _get_project_agent_md_path(self) -> Path | None:
        """Get the path to the project agent.md file.
        
        Returns:
            Path to the project agent.md file if project root is set, None otherwise.
        """
        ...
        if preferred_path.exists():
            ...
            return preferred_path  # Return preferred path even if it doesn't exist
    ...
    def before_agent(
        ...
        if "user_memory" not in state:
            user_path = self._get_user_agent_md_path()
            if user_path.exists():
        ...
        if "project_memory" not in state:
            project_path = self._get_project_agent_md_path()
    ...
    def _build_system_prompt(self, request: ModelRequest) -> str:
        ...
        # Build project superagent directory path
        if self.project_root:
            project_superagent_dir = str(self.project_root / ".superagent")
        else:
            project_superagent_dir = "[project-root]/.superagent (not in a project)"
        ...
        system_prompt += "\n\n" + LONGTERM_MEMORY_SYSTEM_PROMPT.format(
            ...
            project_superagent_dir=project_superagent_dir,
        )
...
def create_agent_memory_middleware(
    assistant_id: str,
    project_root: Path | None = None,
    system_prompt_template: str | None = None,
) -> AgentMemoryMiddleware:
    """Factory function to create an AgentMemoryMiddleware instance.
    
    Args:
        assistant_id: The agent identifier.
        project_root: Optional project root directory for project memory.
        system_prompt_template: Optional custom template for memory injection.
        
    Returns:
        Configured AgentMemoryMiddleware instance.
    """
